...
import numpy as np
import pytest
from pygod.detector import AnomalyDAE, ANOMALOUS, GUIDE
...
def pipeline_checks(detector, X, y):
    ...
    assert hasattr(detector.pipeline.named_steps['detector'], 'decision_score_')
    assert hasattr(detector.regressor, 'reg_R2_')

    assert len(detector.pipeline.named_steps['detector'].decision_score_.numpy()) == len(y)
...
class TestGraphOutlierDetector:
    ...
    def test_model_loading_and_reinitialization(self, tmp_path, outlier_detector, random_graph):
        ...
        new_detector = GraphOutlierDetector(model=str(model_path), regressor_model=str(regressor_path))
        pipeline_checks(detector, X, y)
