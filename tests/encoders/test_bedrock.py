...
from muzlin.encoders import BedrockEncoder

# Code adapted from https://github.com/aurelio-labs/semantic-router/blob/main/tests/unit/encoders/test_bedrock.py


@pytest.fixture
def bedrock_encoder(mocker):
    mocker.patch('muzlin.encoders.bedrock.BedrockEncoder._initialize_client')
...
class TestBedrockEncoder:
    ...
    def test_initialisation_with_session_token(self, mocker):
        mocker.patch(
            'muzlin.encoders.bedrock.BedrockEncoder._initialize_client'
        )
        bedrock_encoder = BedrockEncoder(
            access_key_id='fake_id',
            secret_access_key='fake_secret',
    ...
    def test_failed_client_initialisation(self, mocker):
        ...
        with pytest.raises(ValueError):
            BedrockEncoder(access_key_id='fake_id',
                           secret_access_key='fake_secret')
    ...
    def test_call_method(self, bedrock_encoder):
        ...
        assert result == [[0.1, 0.2, 0.3]
                          ], 'Embedding should be [0.1, 0.2, 0.3]'
    ...
    def test_chunking_functionality(self, bedrock_encoder):
        ...
        assert isinstance(
            chunked_docs, list), 'Chunked result should be a list'
        assert (
    ...
    def test_get_env_variable(self):
        ...
        assert (
            BedrockEncoder.get_env_variable(
                'NON_EXISTENT_VAR', None, default_value)
            == default_value
        )
...
class TestBedrockEncoderWithCohere:
    ...
    def test_cohere_input_type(self, bedrock_encoder_with_cohere):
        ...
        result = bedrock_encoder_with_cohere(
            ['test with different input type'])
        assert isinstance(result, list), 'Result should be a list'
