...
from muzlin.encoders import MistralEncoder

# Code adapted from https://github.com/aurelio-labs/semantic-router/blob/main/tests/unit/encoders/test_mistral.py
...
class TestMistralEncoder:
    def test_mistral_encoder_import_errors(self):
        ...
        assert (
            ...
            '`pip install mistralai`' in str(error.value)
        )
    ...
    def test_mistralai_encoder_call_success(self, mistralai_encoder, mocker):
        ...
        mock_response = EmbeddingResponse(
            ...
            model='mistral-embed',
            object='list',
            usage=UsageInfo(prompt_tokens=1, total_tokens=20,
    ...
    def test_mistralai_encoder_call_with_retries(self, mistralai_encoder, mocker):
        ...
        with pytest.raises(ValueError) as e:
            mistralai_encoder(['test document'])
        assert 'No embeddings returned from MistralAI: Test error' in str(
            e.value)
